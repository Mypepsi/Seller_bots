...
class BuffGeneral(SteamManager):
    ...
    def balance_transfer(self):  # Global Function (class_for_many_functions)
        ...
        while True:
            ...
            try:
                amount_to_withdraw = self.content_database_settings['DataBaseSettings']['Buff_Seller'][
                    'Buff_Seller_transfer_amount']
            except:
            ...
            if amount_to_withdraw:
                for acc_info in self.content_acc_settings_list:
                    ...
                    try:
                        ...
                        try:
                            current_balance_url = f'{self.site_url}/api/asset/get_brief_asset/?_={int(time.time() * 1000)}'
                            response = requests.get(current_balance_url, headers=headers, cookies=buff_cookie, timeout=15).json()
                            response_money = float(response['data']['cash_amount'])
                        except:
                            response_money = None
