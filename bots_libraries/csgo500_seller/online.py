...
class CSGO500Online(SessionManager):
    ...
    def visible_store_inventory(self):
        try:
            ...
            my_inventory_response = requests.get(my_inventory_url, headers=self.csgo500_jwt_apikey, timeout=15).json()
        ...
        for item in my_inventory:
            if 'tradable' in item and item['tradable'] is True and 'enabled' in item and item['enabled'] is True:
    ...
    def visible_store_listed(self):
        ...
        if items_on_sale and len(items_on_sale) != 0:
            try:
                another_apis_list = self.search_in_merges_by_username(self.steamclient.username)['csgo500 parse']
            except:
            ...
            if another_apis_list:
                ...
                payload = {"pagination": {"limit": 500}, "filters": {"appId": 730}, "search": hash_name}

                try:
                    search_url = f'{self.site_url}/api/v1/market/shop'
                    search_response = requests.post(search_url, headers=another_csgo500_jwt_apikey, json=payload, timeout=15).json()
